<x-app-layout>
    ...
    {{-- Filter Section --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-5 mb-6" x-data="{ mode: '{{ $mode }}' }">

        {{-- Tab Headers --}}
        <div class="flex border-b border-gray-200 mb-5">
            <button @click="mode = 'tanggal'" :class="mode === 'tanggal' ? 'border-red-500 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                    class="px-5 py-3 text-sm font-bold border-b-2 transition-colors -mb-px">
                📊 Filter by Tanggal
            </button>
            <button @click="mode = 'lapak'" :class="mode === 'lapak' ? 'border-red-500 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                    class="px-5 py-3 text-sm font-bold border-b-2 transition-colors -mb-px">
                📍 Filter by Lapak
            </button>
        </div>

        {{-- Tab 1: Filter by Tanggal (Rentang) --}}
        <form x-show="mode === 'tanggal'" action="{{ route('reports.sales') }}" method="GET">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ $startDate }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-red-500 focus:ring-red-500 text-sm">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Sampai Tanggal</label>
                    <input type="date" name="end_date" value="{{ $endDate }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-red-500 focus:ring-red-500 text-sm">
                </div>
                <div class="flex items-end gap-2">
                    <button type="submit" class="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg shadow-md transition-colors h-[42px] flex items-center justify-center text-sm">
                        🔍 Tampilkan
                    </button>
                    <a href="{{ route('reports.sales.pdf', ['mode' => 'tanggal', 'start_date' => $startDate, 'end_date' => $endDate]) }}" target="_blank" class="px-4 py-2 bg-gray-800 hover:bg-gray-900 text-white font-bold rounded-lg shadow-md transition-colors h-[42px] flex items-center justify-center text-sm">
        ...
        {{-- Tab 2: Filter by Lapak --}}
        <form x-show="mode === 'lapak'" action="{{ route('reports.sales') }}" method="GET">
            <input type="hidden" name="mode" value="lapak">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div class="sm:col-span-2">
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Pilih Lapak</label>
                    <select name="stall_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-red-500 focus:ring-red-500 text-sm" required>
                        <option value="">— Pilih Lapak —</option>
                        @foreach($stalls as $stall)
                            <option value="{{ $stall->id }}" {{ (string) $stallId === (string) $stall->id ? 'selected' : '' }}>
                                {{ $stall->tempat }} — {{ \Carbon\Carbon::parse($stall->tanggal)->translatedFormat('d F Y') }}
                                @if($stall->status === 'aktif') 🟢 Aktif @else 🔴 Selesai @endif
                            </option>
                        @endforeach
                    </select>
                </div>
                <div class="flex items-end gap-2">
                    ...
                    @if($mode === 'lapak' && $stallId)
                    <a href="{{ route('reports.sales.pdf', ['mode' => 'lapak', 'stall_id' => $stallId]) }}" target="_blank" class="px-4 py-2 bg-gray-800 hover:bg-gray-900 text-white font-bold rounded-lg shadow-md transition-colors h-[42px] flex items-center justify-center text-sm">
                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        PDF
                    </a>
                    @endif
        ...
        <div class="mt-4 flex flex-wrap gap-2 text-xs">
            @if($mode === 'lapak' && $selectedStall)
                <span class="px-2.5 py-1 bg-blue-50 text-blue-700 rounded-full font-semibold">📍 {{ $selectedStall->tempat }}</span>
                <span class="px-2.5 py-1 bg-gray-100 text-gray-600 rounded-full font-medium">{{ \Carbon\Carbon::parse($selectedStall->tanggal)->translatedFormat('d F Y') }}</span>
                <span class="px-2.5 py-1 {{ $selectedStall->status === 'aktif' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700' }} rounded-full font-semibold">{{ $selectedStall->status === 'aktif' ? '🟢 Aktif' : '🔴 Selesai' }}</span>
            @elseif($mode === 'tanggal' && $startDate)
                <span class="px-2.5 py-1 bg-red-50 text-red-700 rounded-full font-semibold">📊 Rentang Tanggal</span>
                    ...
                    @foreach($rekapProduk as $nama => $data)
                        ...
                        <div>
                            <div class="flex justify-between items-end mb-1">
                                ...
                                <span class="text-xs font-bold text-gray-900">{{ $data['qty'] }} botol</span>
                            </div>
